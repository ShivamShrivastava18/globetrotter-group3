...
export default function TripsPage() {
  ...
  const filteredTrips = useMemo(() => {
    const uniqueTrips = trips.reduce((acc, trip) => {
      if (!acc.some((existingTrip) => existingTrip.id === trip.id)) {
        acc.push(trip)
      }
      return acc
    }, [] as Trip[])

    let filtered = uniqueTrips.filter(
      (trip) =>
  ...
  const formatDateRangeOld = (startDate: string, endDate: string) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTrips.map((trip, index) => {
              ...
              const dateInfo = {
                ...
                duration: `${Math.ceil((new Date(trip.end_date).getTime() - new Date(trip.start_date).getTime()) / (1000 * 3600 * 24)) + 1} days`,
              }
